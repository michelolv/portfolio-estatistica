...
# CSS customizado----
css_custom <- "
  :root { --nav-accent: #2563eb; --underline-color: #9ca3af; --panel-bg: rgba(15,23,42,.82); --panel-border: rgba(255,255,255,.10); }
  ...
  /* Painel some ao rolar, so aparece perto do topo ---- */
...
grafico_barras <- function() {
  ...
  p <- ggplot(df, aes(x = reorder(metrica, valor), y = valor, fill = valor)) +
    ...
    theme(
      ...
      panel.grid.major  = element_line(color = "rgba(255,255,255,.08)"),
...
grafico_rede <- function() {
  ...
  arestas <- merge(arestas, nos, by.x = "de", by.y = "id"); names(arestas)[3:4] <- c("x1","y1")
  arestas <- merge(arestas, nos, by.x = "para", by.y = "id"); names(arestas)[5:6] <- c("x2","y2")
...
bloco_projeto <- function(proj) {
  ...
  div(
    ...
    fluidRow(
      ...
      column(
        width = 5,
        plotlyOutput(grafico_id, height = "260px"),
        div(class = "side-actions",
            tags$a(class = "btn-cta", href = proj$link_github, target = "_blank", icon("github"), " Repositório"),
            if (!is.na(proj$link_demo))
              tags$a(class = "btn-cta", href = proj$link_demo, target = "_blank", icon("up-right-from-square"), " Demo")
        )
      )
    )
  )
  ...
  div(
    ...
    fluidRow(
      column(
        ...
        tags$hr(),
        p(cert$descricao),
        h5("Skills:"), badge_tech(cert$skills),
        div(class = "left-actions",
